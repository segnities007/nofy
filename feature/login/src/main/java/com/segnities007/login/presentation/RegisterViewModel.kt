...
internal fun registerViewModelFactory(
    ...
    initializer {
        RegisterViewModel(
            registerPasswordUseCase = RegisterPasswordUseCase(authRepository),
            prepareBiometricEnrollmentOperation = PrepareBiometricEnrollmentOperation(
                biometricCipher = biometricCipher
            ),
            authenticateWithCryptoOperation = AuthenticateWithCryptoOperation(
                biometricHandler = biometricHandler
            ),
            encryptBiometricSecretOperation = EncryptBiometricSecretOperation(
                biometricCipher = biometricCipher
            ),
            saveBiometricSecretUseCase = SaveBiometricSecretUseCase(authRepository)
        )
    }
}
