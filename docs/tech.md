...
アプリ用パスワードは「ただ空でなければよい」とは扱いません。`shared:auth` の password policy を通して最小 15 文字と blocklist を強制し、同一文字の繰り返しや単純な連番も reject します。`platform:crypto` の `PasswordHasher` は NFC 正規化した入力を Argon2id で計算したあと、Android Keystore 保護の HMAC key で pepper 化して保存します。これは NIST が推奨する「salt + iterated hash + optional keyed hash」を、ローカル vault の threat model に合わせて強めたものです。
...
### `DependencyVerificationException` が出たとき

#### 1) Android Studio 同期で `-sources.jar` / `gradle-*-src.zip` がまとめて落ちる

IDE が **Kotlin DSL のソース解決**（`prepareKotlinBuildScriptModel` → `SourceDistributionResolver`）で、プラグインの **`-sources.jar`** や **Gradle 配布の `gradle-9.x.x-src.zip`** を取りに行く。これらは **実行 classpath ではない** ので、`compileKotlin` だけ `--write-verification-metadata` しても登録されず、検証が失敗することがある。

対策: ルートの `gradle/verification-metadata.xml` の `<configuration>` に **`<trusted-artifacts>`** を置き、Gradle 公式どおり `.*-sources[.]jar` / `.*-javadoc[.]jar` /（本リポジトリでは）`.*-src[.]zip` を信頼する。本プロジェクトでは既にその形で入っている。

#### 2) `:build-logic:convention:compileClasspath` などバイナリの SHA が古い

`libs.versions.toml` の **AGP / Kotlin / KSP** などを上げた直後や、キャッシュが入れ替わったあとに、登録済み SHA256 と実体がずれると検証が落ちます。`includeBuild("build-logic")` もルートと別解決になるため、**build-logic 用の classpath** がメタデータに無い・古い、という形で 10〜数十件まとめて失敗することがあります。

**対処（信頼できるネットワークで実行し、差分を人間がレビューしてからコミット）**

1. build-logic の compile classpath を解決させつつメタデータを追記・更新する:
   ```bash
   ./gradlew --write-verification-metadata sha256 :build-logic:convention:compileKotlin
   ```
2. まだ足りない場合は、ルートで依存を広く引くタスクを併用する（例）:
   ```bash
   ./gradlew --write-verification-metadata sha256 :app:assembleDebug
   ```
3. 生成された `gradle/verification-metadata.xml` の差分を確認し、意図したバージョンの artifact だけが増えていることを確認する。

**一時的にビルドだけ通したい場合**（メタデータ更新の代替にはしない）: `./gradlew --dependency-verification lenient <task>` で検証失敗を警告に落とせる（Gradle 9 では `-F lenient` でも可）。

#### 3) 単一の `*.module` / `*.pom` / `*.jar` が無い（例: `org.apache.groovy:groovy:4.0.29`）

Gradle は **Gradle Module Metadata**（`*.module`）も検証対象にできる（`<verify-metadata>true</verify-metadata>` 時）。IDE やツールチェーンが **新しい推移依存**（例: Groovy 4 は `org.apache.groovy`、3 系は `org.codehaus.groovy`）を引くと、メタデータにその版が無くて 1 件だけ落ちることがある。

[公式のトラブルシュート](https://docs.gradle.org/current/userguide/dependency_verification.html#sec:troubleshooting-verification) にあるとおり、信頼できるらしい artifact なら **メタデータを更新**する。

**推奨手順**

1. 失敗した **同じ解決**を起こすタスクに対して、ルートで次を実行して `gradle/verification-metadata.xml` に追記・更新する（差分を必ずレビュー）:
   ```bash
   ./gradlew --write-verification-metadata sha256 prepareKotlinBuildScriptModel
   ```
   他の detached 解決なら、ビルドログに出たタスク名や `build/reports/dependency-verification/.../dependency-verification-report.html` を手がかりに、**その依存を引くタスク**を指定する。
2. **`<component>` を 1 件ずつ手で足す運用は避ける。** 推移依存（例: `groovy` の次に `groovy-ant`）までメタデータが無いと、その都度検証が落ちる。これは Gradle の「性質」というより、**メタデータが断片化した状態**の帰結である。正は最初から（または更新時に）次のように **Gradle にマージさせる**こと:
   ```bash
   ./gradlew --write-verification-metadata sha256 prepareKotlinBuildScriptModel :build-logic:convention:compileKotlin :app:compileDebugKotlin --rerun-tasks
   ```
   生成差分をレビューしてコミットする。
3. 手動追記がどうしても必要な例外なら、レポートの GAV に従い **Maven Central 等から同一ファイルを取得**し `sha256sum` で確認のうえ `<component>` を足す（通常は 2 のみで足りる）。
...
Gradle module path は責務を表す `platform:*` / `shared:*` / `feature:*` を使います。`platform/*`・`shared/*` のディレクトリ名は Gradle モジュール名と一致させています（`:platform:storage` → `platform/storage/`）。
...
ノート機能は `feature:note` で `HorizontalPager` を使った単一ユーザー向けの動的ページ UI を提供します。各ページは markdown 編集 / preview を切り替えられ、スクロール時には floating な TopBar / BottomBar を自動で出し分けます。永続化時は Room + SQLCipher で DB 全体を暗号化し、さらにノート本文は AES-GCM で field encryption したうえで保存します。この field encryption key は Android Keystore に常駐させず、アプリ用パスワードから導出した KEK で wrap した session key を unlock 時だけメモリへ展開します。既存の legacy Keystore 鍵で暗号化された本文は読めた時点で新方式へ再暗号化し、段階的に移行します。`shared:auth` の `lock/unlock/reset` と同じタイミングで `DataCipher` も開閉するため、アプリが lock 状態のときは本文の追加復号・再暗号化を拒否します。ページタイトルは本文先頭から導出し、平文カラムとしては保持しません。
