...
生体認証鍵は `setInvalidatedByBiometricEnrollment(true)` を前提に扱います。つまり enrollment 変更や secure lock 設定変更で鍵が使えなくなるのは正常系です。`feature:login` はこの状態を generic failure にせず、「保存済み biometric secret を破棄して再登録を要求する」typed flow として扱います。
...
オフラインアプリのため、server-side verify を前提にした `Play Integrity` は採用しません。その代わり release build では app root で risky environment detection を行い、debugger 接続、unexpected debuggable app、test-keys firmware、既知の root artifact / root manager を検出した場合は起動自体を block します。これは完全な耐タンパーではなく heuristic ですが、オフライン前提で取り得る現実的な hardening として扱います。
...
機密入力や破壊的操作の UI では、design system component に obscured-touch 防御を持たせます。password field、password 更新ボタン、app reset confirm のような操作は、`FLAG_WINDOW_IS_OBSCURED` / `FLAG_WINDOW_IS_PARTIALLY_OBSCURED` を検知した touch を受け付けません。password field は `KeyboardType.Password` と `autoCorrectEnabled = false` を標準にし、IME の予測変換や通常テキスト向け補完へ寄らない入力設定を使います。
...
release build では R8 による code shrinking / resource shrinking を有効にし、機密ロジックの reverse engineering コストを下げる方向ではなく、上げる方向へ寄せます。debug 用の fallback log は release path へ残しません。
