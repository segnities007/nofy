...
秘密データを扱う都合上、app root では `FLAG_SECURE` と process lifecycle による auto-lock を必須とします。window の secure 化は `MainActivity` で 1 回だけ行い、app が background に入ったら `shared:auth` を通して DB を lock し、navigation も login へ戻します。
